			  ...
			  int ap_seg1_idx, int *bandwidth, int *seg1_idx)
{
	...
	if (sta == NULL) {
		wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_INFO, "Failed to get "
				"STA info to validate the received OCI element");
		return -1;
	}
...
static int ocv_oci_add(struct wpa_state_machine *sm, u8 **pos)
{
	return 0;
}
...
SM_STATE(WPA_PTK_GROUP, REKEYESTABLISHED)
{
	...
	if (wpa_parse_kde_ies(key_data, key_data_length, &kde) < 0) {
		wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_INFO,
				 "received EAPOL-Key msg 1/2 with invalid "
				 "Key Data contents");
		return;
	}
