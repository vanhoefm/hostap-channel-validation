...
class APConnection:
    ...
    def __init__(self, apdev, dev, params):
        ...
        params.update(hostapd.wpa2_params(ssid=self.ssid, passphrase=self.passphrase))
        ...
        dev.connect(self.ssid, raw_psk=self.psk, scan_freq=freq, ocv=sta_ocv, ieee80211w="1", wait_connect=False)
...
def test_wpa2_ocv_ap_mismatch(dev, apdev):
    params = { "channel": "1", "ieee80211w": "1", "freq": "2412"}
...
def test_wpa2_ocv_ap_vht80_mismatch(dev, apdev):
    params = { "hw_mode": "a", "channel": "36", "country_code": "US",
               ...
               "ieee80211ac": "1", "vht_oper_chwidth": "1", "freq": "5180",
...
def test_wpa2_ocv_ap_vht160_mismatch(dev, apdev):
    params = { "hw_mode": "a", "channel": "100", "country_code": "ZA",
               "ht_capab": "[HT40+]", "ieee80211w": "1", "ieee80211n": "1",
               "ieee80211ac": "1", "vht_oper_chwidth": "2", "freq": "5500",
               "vht_oper_centr_freq_seg0_idx": "114", "ieee80211d": "1",
               "ieee80211h": "1" }
    conn = APConnection(apdev[0], dev[0], params)
...
def test_wpa2_ocv_ap_vht80plus80_mismatch(dev, apdev):
    params = { "hw_mode": "a", "channel": "36", "country_code": "US",
               "ht_capab": "[HT40+]", "ieee80211w": "1", "ieee80211n": "1",
               "ieee80211ac": "1", "vht_oper_chwidth": "3", "freq": "5180",
               "vht_oper_centr_freq_seg0_idx": "42", "ieee80211d": "1",
               "vht_oper_centr_freq_seg1_idx": "155", "ieee80211h": "1" }
    conn = APConnection(apdev[0], dev[0], params)
...
def test_wpa2_ocv_ap_unexpected1(dev, apdev):
    params = { "channel": "1", "ieee80211w": "1", "ap_ocv": "0",
               "sta_ocv": "1", "freq": "2412" }
    conn = APConnection(apdev[0], dev[0], params)
...
def test_wpa2_ocv_ap_unexpected2(dev, apdev):
    params = { "channel": "1", "ieee80211w": "1", "ap_ocv": "1",
               "sta_ocv": "0", "freq": "2412" }
    conn = APConnection(apdev[0], dev[0], params)
    logger.debug("Client will send OCI KDE even if it was not negotiated")
    conn.confirm_valid_oci(81, 1, 0)
...
def test_wpa2_ocv_ap_retransmit_msg3(dev, apdev):
    """Verify that manually retransmitted Msg 3/4's also contain a correct OCI"""
    bssid = apdev[0]['bssid']
    ssid = "test-wpa2-ocv"
    passphrase = "qwertyuiop"
    ...
    dev[0].connect(ssid, psk=passphrase, scan_freq="2412", wait_connect=False, ocv="1", ieee80211w="1")
    addr = dev[0].own_addr()
...
def test_wpa2_ocv_ap_grouphs(dev, apdev):
    params = { "channel": "1", "ieee80211w": "1", "freq": "2412", "wpa_strict_rekey": "1" }
    conn = APConnection(apdev[0], dev[0], params)
    conn.confirm_valid_oci(81, 1, 0)

    conn.hapd.request("SET ext_eapol_frame_io 0")
    dev[1].connect(conn.ssid, psk=conn.passphrase, scan_freq="2412", ocv="1", ieee80211w="1")
...
class STAConnection:
    ...
    def __init__(self, apdev, dev, params, sta_params=None):
        ...
        self.dev.connect(self.ssid, raw_psk=self.psk, scan_freq=freq, wait_connect=False, **sta_params)
        self.addr = dev.p2p_interface_addr()
...
def test_wpa2_ocv_mismatch_client(dev, apdev):
    ...
    conn.test_bad_oci("element missing", None, 0, 0, "did not recieve mandatory OCI")
    conn.test_bad_oci("wrong channel number", 81, 6, 0, "primary channel mismatch")
    conn.test_bad_oci("invalid channel number", 81, 0, 0, "unable to interpret received OCI")
    conn.test_bad_oci("wrong operating class", 80, 0, 0, "unable to interpret received OCI")
    conn.test_bad_oci("invalid operating class", 0, 0, 0, "unable to interpret received OCI")
    conn.confirm_valid_oci(81, 1, 0)
...
def test_wpa2_ocv_sta_grouphs(dev, apdev):
    params = { "channel": "1", "ieee80211w": "1", "ocv": "1", "freq": "2412",
               "wpa_strict_rekey": "1" }
